...
/* Line 85 of "BIFParser.y"  */
  ...
  public BayesianNetwork parseNetwork() throws IOException {
	  network = new base.BayesianNetwork();
	  if (parse()) {
		  return network;
	  } else {
		  return null;
  ...
  protected void defineVariable(String name, VariableContentItemList items) {
	  ...
	  if (network != null) {
		  Domain domain = new base.Domain(items.size());
		  for (VariableContentItem item : items) {
			  if (item instanceof VariableDiscrete) {
          VariableDiscrete vditem = (VariableDiscrete)item;
          if (vditem.values.get(0).equalsIgnoreCase("true")) {
            domain = new base.BooleanDomain();
          } else { 
				    for (String value : vditem.values) {
					    domain.add(new base.StringValue(value.toLowerCase()));
            }
          }
			  }
		  }
		  RandomVariable variable = new base.NamedVariable(name, domain);
		  ...
		  for (ProbabilityContentEntry entry : entries) {
			  if (entry instanceof ProbabilityEntry) {
				  ...
				  for (RandomVariable given : givens) {
					  Value val = new base.StringValue(values.next());
					  a.put(given, val);
				  }
